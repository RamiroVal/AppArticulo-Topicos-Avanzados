	...
	public void actionPerformed(ActionEvent e) {
		...
		if (evento.equals("Consulta X Articulo")) { //Si la opción seleccionada fue la de Consulta X Articulo
			if (marca.getNombreMarcas().length == 0) { //Comprueba que ya se hayan agregado marcas previamente
				...
				//Muestra la primera opción del JComboBox cuando se abre
				String[][] datos = tabla.getTabla();
				consultar.setLblClave(datos[0][0]);
				consultar.setLblNombre(datos[0][1]);
				consultar.setLblMarca(datos[0][2]);
				consultar.setLblExistencia(datos[0][3]);
				consultar.setLblPrecio(datos[0][4]);
